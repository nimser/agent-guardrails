# Delta for Claude Code Adapter

## ADDED Requirements

### Requirement: Shell Hook Script
The system MUST implement a shell hook script for Claude Code.
...
- WHEN Claude Code invokes the hook
...
- THEN hook MUST exit with code 2 and JSON `decision: block`
...
The system MUST provide settings.json for hook configuration.

#### Scenario: settings.json structure
- WHEN settings.json is present
- THEN it MUST contain PreToolUse and PostToolUse hook paths
