...
import type { ToolCallContext, RulePack, GuardrailRule, HarnessCapabilities } from '../core/types.js';
...
describe('matchAndResolve', () => {
  ...
  it('applies capability fallback (run → suggest)', () => {
    ...
    const noRun: HarnessCapabilities = { block: true, suggest: true, run: false, redact: false, confirm: false };
...
describe('getStats', () => {
  ...
  it('stats increment after matchAndResolve calls', () => {
    ...
    const caps: HarnessCapabilities = { block: true, suggest: true, run: true, redact: true, confirm: true };
