...
describe('file-path matcher', () => {
  it('returns true when pattern matches filePath', () => {
    ...
    expect(filePathHandler.matches(matcher as any, ctx)).toBe(true);
  });
  ...
  it('returns false when pattern does not match filePath', () => {
    ...
    expect(filePathHandler.matches(matcher as any, ctx)).toBe(false);
  });

  it('returns false when filePath is missing', () => {
    const matcher: GuardrailMatcher = { type: 'file-path', pattern: /test/i };
